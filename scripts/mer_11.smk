...
param_configfile = config['param_grid_configfile']
...
rule model_training:
    ...
    shell:
        "python scripts/model_creation/11mer_train.py  \
                --modelname {wildcards.model_name} \
                --paramconfig {params.param_grid_file} \
                --matrix {input.matrix} \
                --rows {input.rows} \
                --cols {input.cols} \
                --labels {input.labels} \
...
rule top_feat_table:
    input:
        ...
        feat_order = "results/wgs_standard/{k}mer/top_feats/{model_name}/top_feats_order.npy"
    output:
        "results/wgs_standard/{k}mer/top_feats/{model_name}/feat_count_table.tsv"
    run:
        # goal is a table of the format:
        #  genome     kmer1     kmer2    kmer 3
        #  ERR123     11        0         2
        # order of the columns
        feat_order = np.load(input.feat_order)
        # better to make a dataframe from a list than
        individual_dfs = []
        # for each file
        for f in input.query_files:
            ...
            #id = id.split(".txt")[0]
            # load the tsv
            jf = pd.read_csv(f,header=None,sep='\t')
            print(jf)
            # transpose it?
            # assert column order
            # add the name to the row
            # append it to the list of dfs
            sys.exit()
